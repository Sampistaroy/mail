	...
	<div class="container">
		...
		<div class="row">
			    ...
			    <div class="post blog-block">
					<h2 id="post-<?php the_ID(); ?>" <?php $classes = array('align-left','social-magazine-one-post-link',); post_class( $classes ); ?>><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
						...
						<?php the_excerpt(); ?>
							<p class="posted-in">
								<small>
								<?php if (has_category()) { ?><?php esc_attr_e('Posted in ', 'social_magazine' ); ?><?php the_category(', ') ?><?php } ?>
								<?php the_tags(__('Tagged with: ','social_magazine')); ?>
								</small>
							</p>
				</div><!-- /blog-block -->
			...
			<?php endwhile; ?>
				...
				<div class="alignleft"><?php previous_posts_link( __( '&#8592; Previous Article', 'social_magazine' ) ); ?></div>
				<div class="alignright"><?php next_posts_link( __( 'Next Articles &#187;', 'social_magazine' ) ); ?></div>
